...
class wpTwitter {
	...
	private function _get_request_defaults() {
		$params = array(
			'sslverify' => apply_filters( 'attp_sslverify', false ),
			'body'      => array(
				'oauth_version'      => '1.0',
				'oauth_nonce'        => md5( microtime() . mt_rand() ),
	...
	public function send_authed_request( $request_url, $method, $body_parameters = array() ) {
		...
		switch ($method) {
			case 'GET':
				$request_url = $this->get_normalized_http_url( $request_url ) . '?' . attpOAuthUtil::build_http_query( $parameters['body'] );
		...
		if ( !is_wp_error( $resp ) && $resp['response']['code'] >= 200 && $resp['response']['code'] < 300 ) {
			 ...
			 * @see https://github.com/OpenAjaxtown/twitter-widget-pro/pull/8
			 */
	...
	*/
	public function get_signable_parameters( $parameters ) {
		...
		return attpOAuthUtil::build_http_query( $parameters );
	}
	...
	public function build_signature( $parameters, $request_url, $method = 'GET' ) {
		...
		$parts = attpOAuthUtil::urlencode_rfc3986($parts);
		...
		$key_parts = attpOAuthUtil::urlencode_rfc3986( $key_parts );
