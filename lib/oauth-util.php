...
class attpOAuthUtil {
	public static function urlencode_rfc3986( $input ) {
		if ( is_array( $input ) )
			return array_map( array( 'attpOAuthUtil', 'urlencode_rfc3986' ), $input );
	...
	public static function split_header($header, $only_allow_oauth_parameters = true) {
		...
		while ( preg_match( $pattern, $header, $matches, PREG_OFFSET_CAPTURE, $offset ) > 0 ) {
			...
			if ( preg_match( '/^oauth_/', $header_name ) || ! $only_allow_oauth_parameters )
				$params[$header_name] = attpOAuthUtil::urldecode_rfc3986($header_content);
	...
	public static function parse_parameters( $input ) {
		...
		foreach ( $pairs as $pair ) {
			...
			$parameter = attpOAuthUtil::urldecode_rfc3986( $split[0] );
			$value = isset( $split[1] ) ? attpOAuthUtil::urldecode_rfc3986( $split[1] ) : '';
	...
	public static function build_http_query($params) {
		...
		$keys = attpOAuthUtil::urlencode_rfc3986( array_keys( $params ) );
		$values = attpOAuthUtil::urlencode_rfc3986( array_values( $params ) );
